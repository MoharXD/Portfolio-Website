...
const AIOrbFace = ({
  ...
  const openScale = (() => {
    ...
    if (state === "streaming") return 0.88;
    return 1;
  })();
  ...
  const blink = useCallback(
    async function doBlink(double = false) {
  ...
  useEffect(() => {
    if (shouldReduceMotion || !eyesOpen) return;
    let timeout: ReturnType<typeof setTimeout>;
    let mounted = true;
    const schedule = () => {
      timeout = setTimeout(
        () => {
          if (mounted) blink(Math.random() < DOUBLE_BLINK_CHANCE);
          schedule();
        },
  ...
  useEffect(() => {
    if (!gaze || shouldReduceMotion || isThinking || !eyesOpen) return;
  ...
  useEffect(() => {
    if (!isThinking || shouldReduceMotion) return;
    let timeout: ReturnType<typeof setTimeout>;
    let mounted = true;
    let index = 0;
    const schedule = () => {
  ...
  useEffect(() => {
    if (!isHappy || shouldReduceMotion) return;
    bodyControls.start(
      {
  ...
  useEffect(() => {
    if (shouldReduceMotion || !isListening) return;
